#!/usr/bin/env bash
# Re-run of train_sft.sh with intermediate checkpoints every 29 steps (quarter-epoch,
# 12 checkpoints over 348 steps). Same hyperparameters otherwise. New experiment name
# so the original qwen05b-gsm8k-sft-instruct final checkpoint is not clobbered.
# Produces checkpoints at checkpoints/gsm8k-sft/qwen05b-gsm8k-sft-instruct-ckpts/.
set -euo pipefail
...
"$ENV_BIN/torchrun" --standalone --nnodes=1 --nproc-per-node=1 -m verl.trainer.sft_trainer \
    ...
    trainer.experiment_name=qwen05b-gsm8k-sft-instruct-ckpts \
